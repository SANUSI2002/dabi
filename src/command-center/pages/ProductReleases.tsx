...
import { ecosystemOfProduct, PRODUCT_ECOSYSTEM_FILTERS } from "../productEcosystem";
...
const contextToEcosystemFilter: Record<string, (typeof PRODUCT_ECOSYSTEM_FILTERS)[number]> = { all: "All", "sabi-os": "Sabi OS", "sabi-health": "Sabi Health" };

export default function ProductReleases() {
  const command = useCommandCenter();
  const roadmap = useRoadmap();
  const productContext = useProductContext((state) => state.product);
  const userId = useAuth((state) => state.identity?.platformUserId);
  const user = command.platformUsers.find((entry) => entry.id === userId);
  const [query, setQuery] = useState("");
  const [ecosystem, setEcosystem] = useState<(typeof PRODUCT_ECOSYSTEM_FILTERS)[number]>(contextToEcosystemFilter[productContext] ?? "All");
  ...
  const filtered = releases.filter((release) => {
    ...
    return (ecosystem === "All" || ecosystemOfProduct(release.productId) === ecosystem)
      && (productId === "all" || release.productId === productId)
  ...
  return <div>
    ...
    <Modal open={!!form} onClose={() => { setForm(null); setError(""); }} title={editingId ? "Edit release" : "Create release"} wide footer={<><CommandButton variant="secondary" onClick={() => setForm(null)}>Cancel</CommandButton><CommandButton onClick={save}>{editingId ? "Save changes" : "Create release"}</CommandButton></>}>
      {form && <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Product"><CommandSelect className="w-full" value={form.productId} disabled={!!editingId} onChange={(event) => setForm({ ...form, productId: event.target.value })}>{["Sabi OS", "Sabi Health", "Shared"].map((group) => { const items = command.products.filter((product) => ecosystemOfProduct(product.id) === group); return items.length > 0 ? <optgroup key={group} label={group}>{items.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</optgroup> : null; })}</CommandSelect></Field>
