...
import { Bell, Command, LogOut, Menu, Search, ShieldCheck, X } from "lucide-react";
import { COMMAND_NAV } from "../navigation";
...
function CommandSidebar({ close }: { close?: () => void }) {
  ...
  const nav = useMemo(() => COMMAND_NAV.map((group) => ({ ...group, items: group.items.filter((item) => !item.permission || hasPermission(user, item.permission)) })).filter((group) => group.items.length), [user]);
  return (
    <aside className="flex h-full w-[252px] shrink-0 flex-col border-r border-white/[0.07] bg-[#09130f] text-slate-300">
      <div className="flex h-[68px] items-center gap-3 border-b border-white/[0.07] px-4">
        ...
        <div className="leading-tight"><p className="font-display text-sm font-extrabold tracking-tight text-white">SABI OS</p><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-emerald-400">Command Center</p></div>
...
export function CommandCenterShell() {
  ...
  useEffect(() => {
    ...
    document.title = "Sabi OS Command Center";
    ...
    <div className="flex h-full min-h-screen bg-[#f4f7f5] text-slate-900">
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-30 flex h-[68px] items-center gap-3 border-b border-slate-200/80 bg-white/90 px-4 backdrop-blur-xl lg:px-6">
          ...
          <div className="ml-auto flex items-center gap-2"><span className="hidden rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700 ring-1 ring-emerald-200 sm:inline">All systems monitored</span><button onClick={() => navigate("/command-center/notifications")} className="relative rounded-lg p-2 text-slate-500 hover:bg-slate-100" aria-label="Notifications"><Bell size={17} /><span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-amber-500 ring-2 ring-white" /></button><button onClick={() => { signOut(); window.location.assign("/login"); }} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100" aria-label="Sign out of Command Center"><LogOut size={17} /></button></div>
        </header>
