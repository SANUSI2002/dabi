...
import { Plus, Percent, FileCheck2, Banknote } from "lucide-react";
...
import type { TaxKind } from "@/data/accounting/tax";

export default function TaxCenter() {
  const { rates, returns, addRate, updateRate, prepareReturn, fileReturn, payReturn } = useTax();
  ...
  const [pf, setPf] = useState({ start: isoDate(new Date(Date.now() - 30 * 864e5)), end: isoDate(new Date()) });

  const netDue = returns.filter((r) => r.status !== "Paid").reduce((n, r) => n + Math.max(0, r.netPayable), 0);

  return (
    <div>
      <PageHeader title="Tax" subtitle="Tax rates and VAT returns — output vs input tax computed from the ledger"
        actions={<>
          <Button variant="soft" onClick={() => setRateModal(true)}><Plus size={15} /> Tax Rate</Button>
          <Button onClick={() => setPrep(true)}><FileCheck2 size={15} /> Prepare VAT Return</Button>
        </>} />
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <StatCard label="Net VAT due" value={money(netDue)} tone="action" delay={0.1} />
      </div>

      <Tabs tabs={["VAT Returns", "Tax Rates"]}>
        {(t) =>
          t === "Tax Rates" ? (
            <Card className="p-0">
            ...
            <EmptyState title="No VAT returns yet" hint="Prepare one for a period — it reads the movement on VAT Payable." />
          ) : (
            <Card className="p-0">
              <Table columns={["Reference", "Period", "Output tax", "Input tax", "Net payable", "Status", ""]}>
                {returns.map((r, i) => (
                  <Row key={r.id} index={i}>
                    <Cell className="font-mono text-xs">{r.reference}</Cell>
                    <Cell>{shortDate(r.periodStart)} – {shortDate(r.periodEnd)}</Cell>
                    <Cell className="font-mono">{money(r.outputTax)}</Cell>
                    <Cell className="font-mono">{money(r.inputTax)}</Cell>
                    ...
                    <Cell>
                      <div className="flex justify-end gap-1">
                        {r.status === "Open" && <button className="btn-ghost px-2 py-1 text-xs" onClick={() => fileReturn(r.id)}>File</button>}
      ...
      <Modal open={prep} onClose={() => setPrep(false)} title="Prepare VAT Return"
        footer={<><Button variant="ghost" onClick={() => setPrep(false)}>Cancel</Button><Button onClick={() => { prepareReturn(new Date(pf.start + "T00:00:00Z").toISOString(), new Date(pf.end + "T23:59:59Z").toISOString()); setPrep(false); }}>Prepare</Button></>}>
        <div className="space-y-3">
          <p className="text-sm text-mist-600">Output tax (credits to VAT Payable) minus input tax (debits) over the period.</p>
      ...
      <Modal open={!!payId} onClose={() => setPayId(null)} title="Pay VAT"
