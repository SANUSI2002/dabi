...
export default function EmployeeDetail() {
  ...
  const myAllocations = allocations.filter((a) => a.employeeId === id);
  const myPayslips = payslips.filter((ps) => ps.employeeId === id);
  const myStructure = structureFor(id ?? "");
  const myHistory = emp
          ...
          t === "Overview" ? (
            <div className="grid gap-5 lg:grid-cols-2">
              ...
              <Card>
                ...
                <dl className="space-y-1.5 text-sm">
                  {[
                    ...(companies.length > 1 ? [["Branch", companyName(p?.companyId)]] as const : []),
            ...
            <div className="space-y-3">
              <div className="flex justify-end">
                <Link to="/hr/assets" className="btn-ghost text-xs"><Package size={13} /> Open Company Assets →</Link>
              </div>
              <Table columns={["Asset", "Category", "Tracking ID", "Assigned", "Returned", "Condition"]}>
                {myAllocations.map((a, i) => {
                  const asset = companyAssets.find((x) => x.id === a.assetId);
                  return (
                    <Row key={a.id} index={i}>
                      <Cell className="font-semibold">{asset?.name ?? "—"}</Cell>
                      <Cell className="text-mist-500">{assetCategories.find((c) => c.id === asset?.categoryId)?.name ?? "—"}</Cell>
                      <Cell className="font-mono text-mist-500">{asset?.trackingId ?? "—"}</Cell>
                      <Cell className="text-mist-400">{shortDate(a.assignedDate)}</Cell>
                      <Cell className="text-mist-400">{a.returnDate ? shortDate(a.returnDate) : <Badge tone="brand">Holding</Badge>}</Cell>
                      <Cell>{a.returnCondition ? <Badge tone={a.returnCondition === "Good" ? "mist" : "action"}>{a.returnCondition}</Badge> : "—"}</Cell>
                    </Row>
                  );
                })}
                {myAllocations.length === 0 && <Row><Cell className="text-mist-400">No assets allocated to this employee.</Cell><Cell /><Cell /><Cell /><Cell /><Cell /></Row>}
              </Table>
            </div>
          ) : t === "Payroll" ? (
            <div className="space-y-5">
              <div className="flex justify-end">
                <Link to="/hr/payroll" className="btn-ghost text-xs"><Wallet size={13} /> Open Payroll →</Link>
              </div>
              {myStructure && (
                <Card>
                  <h3 className="mb-3 font-display font-bold text-mist-900">Current salary structure</h3>
                  <dl className="space-y-1.5 text-sm">
                    <div className="flex justify-between border-b border-dashed border-mist-100 py-1.5">
                      <dt className="text-mist-400">Basic salary</dt>
                      <dd className="font-medium text-mist-800">{naira(myStructure.basicSalary)}</dd>
                    </div>
                    {myStructure.allowances.map((c) => (
                      <div key={c.name} className="flex justify-between border-b border-dashed border-mist-100 py-1.5">
                        <dt className="text-mist-400">{c.name}</dt>
                        <dd className="font-medium text-brand-600">+{naira(c.amount)}</dd>
                      </div>
                    ))}
                    {myStructure.deductions.map((c) => (
                      <div key={c.name} className="flex justify-between border-b border-dashed border-mist-100 py-1.5">
                        <dt className="text-mist-400">{c.name}</dt>
                        <dd className="font-medium text-action-600">-{naira(c.amount)}</dd>
                      </div>
                    ))}
                  </dl>
                </Card>
              )}
              <Table columns={["Batch", "Period", "Gross", "Deductions", "Net pay", "Status"]}>
                {myPayslips.map((ps, i) => (
                  <Row key={ps.id} index={i}>
                    <Cell className="font-semibold">{ps.batch}</Cell>
                    <Cell className="text-mist-400">{shortDate(ps.startDate)} – {shortDate(ps.endDate)}</Cell>
                    <Cell>{naira(ps.grossPay)}</Cell>
                    <Cell className="text-action-600">{naira(ps.totalDeductions)}</Cell>
                    <Cell className="font-semibold">{naira(ps.netPay)}</Cell>
                    <Cell><Badge tone={statusTone(ps.status)}>{ps.status}</Badge></Cell>
                  </Row>
                ))}
                {myPayslips.length === 0 && <Row><Cell className="text-mist-400">No payslips generated yet.</Cell><Cell /><Cell /><Cell /><Cell /><Cell /></Row>}
              </Table>
            </div>
          ) : (
            <div className="space-y-2">
              {myHistory.map((e) => (
                <div key={e.id} className="flex items-start justify-between gap-3 rounded-xl bg-mist-50 px-3 py-2.5 text-sm">
                  <span className="flex items-start gap-2">
                    <HistoryIcon size={14} className="mt-0.5 shrink-0 text-mist-400" />
                    <span>
                      <b className="font-semibold text-mist-800">{e.action}</b>
                      <span className="text-mist-500"> · {e.resource}</span>
                      <span className="block text-[11px] text-mist-400">{e.user} ({e.role}) · {timeAgo(e.ts)}</span>
                    </span>
                  </span>
                </div>
              ))}
              {myHistory.length === 0 && <p className="text-sm text-mist-400">No recorded activity for this employee yet.</p>}
            </div>
          )
        }
      </Tabs>
