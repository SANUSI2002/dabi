...
import { liveSignIn, restoreLiveIdentity } from "@/identity/liveIdentity";
...
export default function PharmacyLoginPage() {
  ...
  const [email, setEmail] = useState(developmentFixturesEnabled && !apiConfigured ? DEMO_PHARMACY_EMAIL : "");
  const [password, setPassword] = useState(developmentFixturesEnabled && !apiConfigured ? DEMO_PASSWORD : "");
  const [rememberMe, setRememberMe] = useState(false);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  useEffect(() => { if (apiConfigured) { restoreLiveIdentity().then((identity) => { if (identity) navigate('/identity/account', { replace: true }); }); } }, [navigate]);

  if (!apiConfigured && authed && hasPharmacyPortalAccess(activeMembership)) return <Navigate to="/pharmacy-portal" replace />;
  ...
  async function submit(event: FormEvent) {
    ...
    if (apiConfigured) {
      try {
        ...
        if (result.kind === 'MFA') { navigate('/mfa?context=pharmacy'); return; }
        const identity = result.identity;
        if (!identity.organizations.some((membership) => membership.status === 'ACTIVE' && membership.organization.type === 'PHARMACY')) throw new Error('This identity has no active pharmacy membership.');
