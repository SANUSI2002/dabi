...
export const demoIdentityOptions = SABI_IDENTITIES.map((identity) => ({
  ...
  name: identity.name,
  label: identity.kind === "platform" ? "Platform administrator" : identity.kind === "patient" ? "Patient" : ORGANIZATION_MEMBERSHIPS.filter((membership) => membership.identityId === identity.id).length > 1 ? "Multi-organization administrator" : "Hospital employee",
}));
