...
export function AddMemberPage() {
  ...
  // An invite link (/family/join#code) opens straight into joining that circle.
  const linkCode = location.hash.replace(/^#/, "");
  const joining = location.pathname === "/family/join" || Boolean(linkCode);

  // "add" = add someone to MY circle. "join" = I'm joining SOMEONE ELSE'S circle.
  const [mode, setMode] = useState(joining ? "join" : "add");
  ...
  // Server errors are shown in the page's toast.
  const attempt = async (work) => {
    ...
    try {
      ...
      notify(err.status === 404 ? "We couldn't find a Sabi Health patient with that ID" : err.status === 409 ? "There's already an open invite for this person" : err.message);
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleSendPatientIdRequest = () => {
    ...
    attempt(async () => {
      const patientId = requestForm.patientId.trim();
      const token = await addMember({ patientId, relationship: requestForm.relationship, permission: levelId, access });
      const name = requestForm.fullName.trim() || `Pending Patient (${patientId})`;
      const member = { id: patientId, name, initials: initialsOf(name), color: colorFor(patientId), relationship: requestForm.relationship, sabiHealthId: patientId };
      navigate("/family/add/success", { state: { kind: "requested", member, permission: levelLabel, token } });
    });
  };
  ...
  const handleSendInvite = () => {
    ...
    attempt(async () => {
      const token = await addMember({ email: contact.trim(), permission: levelId, access });
      navigate("/family/add/success", {
        state: { kind: "invite", contact: contact.trim(), permission: levelLabel, token },
      });
    });
  };
  ...
  const handleLookupCode = () =>
    attempt(async () => {
      try {
        setMatchedCircle(await findCircleByCode(inviteCode));
      } catch {
  ...
  const handleConfirmJoin = () => {
    ...
    attempt(async () => {
      await addJoinedCircle(matchedCircle, levelId, access);
      navigate("/family/add/success", {
                ...
                <div className="sabi-fam-coordination-row" style={{ background: "var(--sabi-page-bg)" }}>
                  <div className="who">
                    ...
                    <div>
                      ...
                      <span>Owned by {matchedCircle.ownerName}{matchedCircle.kind === "DIRECT" ? " · You were invited personally" : ""}</span>
                    </div>
                  </div>
                </div>
