...
// Every appointment "belongs" to whoever it was booked for — hospital
// bookings carry a bookedFor name; anything without one is assumed to
// be for the account owner (self).
function buildFamilyEvents(members) {
  return getAppointments()
    .map((apt) => {
      const member = apt.bookedFor
        ? members.find((m) => m.name === apt.bookedFor)
        : members.find((m) => m.isSelf);
      const date = toCalendarDate(apt.date);
      if (!member || !date) return null;
      return { id: apt.id, date, member, title: apt.doctor, time: apt.time, status: apt.status };
    })
    .filter(Boolean)
...
export function CareCalendarPage() {
  ...
  const events = useMemo(() => buildFamilyEvents(members), [members]);
