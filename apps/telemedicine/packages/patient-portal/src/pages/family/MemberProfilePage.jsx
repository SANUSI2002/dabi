...
export function MemberProfilePage() {
  ...
  const [member, setMember] = useState(() => getMember(memberId));
      ...
      <div className="sabi-dashboard" style={{ ...pageVars, zoom }}>
        ...
        <div className="sabi-main">
          ...
          <div className="sabi-card">
            <p>We couldn&apos;t find that member.</p>
  ...
  const setPermissionLevel = (id) => {
    const next = updateMember(member.id, { permission: id, access: DEFAULT_ACCESS_BY_LEVEL[id] });
    setMember(next);
  ...
  const toggleAccess = (key) => {
    ...
    const next = updateMember(member.id, { access: nextAccess });
    setMember(next);
  };
  ...
  const memberAppointments = getAppointments().filter((apt) => {
    if (member.isSelf) return !apt.bookedFor || apt.bookedFor === member.name;
    return apt.bookedFor === member.name;
  });
