...
The free Render service can spin down after inactivity, delaying the first request. The free database expires on 23 October 2026 and has no backups. Replace both before any real use. Public-Git Render service deployments are manual in this configuration; pushing a backend commit does not automatically update this test service. The Vercel projects are CLI-deployed rather than Git-connected, so pushing frontend code does not automatically redeploy them either.
